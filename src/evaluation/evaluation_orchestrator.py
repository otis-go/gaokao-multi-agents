...
class EvaluationOrchestrator:
    ...
    def run(
        ...
        # Keep original object references for in-memory downstream access.
        state.input_record = core
        if stage1_meta is not None:
            state.input_stage1_meta_obj = stage1_meta
    ...
    def _save_evaluation_state(self, state: EvaluationPipelineState) -> None:
        ...
        try:
            ...
            traceback.print_exc()
            # Try to save a minimized error payload.
            try:
                error_payload = {
                    "error": str(e),
                    "unit_id": str(getattr(state, "unit_id", None) or "unknown"),
                ...
                print(f"[WARNING] Fallback error payload saved to: {output_file}")
            except Exception as e2:
                print(f"[WARNING] Failed to save fallback error payload: {e2}")
