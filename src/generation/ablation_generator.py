...
# Chinese LLM prompt - kept for domain-specific Gaokao question generation
ABLATION_ESSAY_PROMPT = """你是一位高考语文命题专家。请根据以下材料，命制一道高考语文阅读理解主观题（简答题）。

【材料】
{material}

【要求】
1. 根据材料设计一个明确的问题，符合主观简答题的特征
2. 提供标准答案要点和评分标准
...
def _normalize_internal_question_type(question_type: str) -> str:
    """
    Normalize external question type labels to ablation generator's internal types.

    Supported inputs:
    - "single-choice" / "essay"
    - Chinese aliases such as "选择题" / "简答题" / "主观题"
    """
    qt = (question_type or "").strip()
    qt_lower = qt.lower()

    if qt_lower in {"single-choice", "choice", "mcq"}:
        return "single-choice"
    if qt_lower in {"essay", "subjective"}:
        return "essay"

    if any(keyword in qt for keyword in ["选择", "单选", "多选"]):
        return "single-choice"
    if any(keyword in qt for keyword in ["简答", "主观", "论述", "问答"]):
        return "essay"

    raise ValueError(
        f"Unsupported ablation question_type={question_type!r}; "
        f"expected single-choice/essay or Chinese aliases."
    )
...
class AblationGenerator:
    ...
    def generate(
        ...
        Args:
            ...
            question_type: Question type ("single-choice" / "essay" or Chinese aliases)
            unit_id: Unit ID
        ...
        """
        internal_qtype = _normalize_internal_question_type(question_type)
        question_type_display = "single-choice" if internal_qtype == "single-choice" else "essay"
        ...
        print(f"[AblationGenerator] Start generation: unit_id={unit_id}, question_type={question_type_display}")
        ...
        print(f"[AblationGenerator] Generation complete: stem_length={len(question.stem)}")
    ...
    def _call_llm(self, prompt: str) -> str:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                logger.warning(f"[AblationGenerator] LLM call failed (attempt {attempt+1}/{self.config.max_retries}): {e}")
    ...
    def _parse_response(
        ...
        try:
            ...
            logger.error(f"[AblationGenerator] JSON parsing failed: {e}")
    ...
    def _create_fallback_question(
        ...
        if question_type == "single-choice":
            return GeneratedQuestion(
                stem=f"[GENERATION_FAILED] {error_msg}",
                question_type="single-choice",
                options=[
                    OptionItem(label="A", content="Option A", is_correct=False),
                    OptionItem(label="B", content="Option B", is_correct=True),
                    OptionItem(label="C", content="Option C", is_correct=False),
                    OptionItem(label="D", content="Option D", is_correct=False),
                ],
                correct_answer="B",
                explanation=f"Generation failed: {error_msg}",
                material_text=material,
            )
        else:
            return GeneratedQuestion(
                stem=f"[GENERATION_FAILED] {error_msg}",
                question_type="essay",
                answer_points=[AnswerPoint(point="Answer point", score=6)],
                total_score=6,
                explanation=f"Generation failed: {error_msg}",
                material_text=material,
            )
...
class AblationOrchestrator:
    ...
    def run_single(self, unit_id: str) -> Dict[str, Any]:
        ...
        if not raw:
            raise ValueError(f"Raw material not found for unit_id={unit_id}")
    ...
    def run_batch(self, unit_ids: List[str]) -> List[Dict[str, Any]]:
        ...
        for i, unit_id in enumerate(unit_ids, 1):
            print(f"\n>>> [{i}/{total}] Processing unit_id={unit_id}...")
            try:
                result = self.run_single(unit_id)
                results.append(result)
                print(f"    Complete: question_type={result['question_type']}")
            except Exception as e:
                logger.error(f"[AblationOrchestrator] Failed to process unit_id={unit_id}: {e}")
                results.append({
                    "unit_id": unit_id,
                    "error": str(e),
                })
